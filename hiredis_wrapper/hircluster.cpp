...
{
    cluster_node_deinit((cluster_node *)val);
...
{
    cluster_slot_destroy((cluster_slot *)val);
}
...
{
    struct cmd *cmd = (struct cmd *)command;
    command_destroy(cmd);
}
...
static void __redisClusterSetError(redisClusterContext *cc, int type, const char *str) {
    ...
    if (str != NULL) {
		...
		__redis_strerror_r(error_number, cc->errstr, sizeof(cc->errstr));
    }
}
...
{   
    ...
    if(node->migrating)
    {
        ...
        {
            oslot = (copen_slot **)hiarray_pop(node->migrating);
    ...
    if(node->importing)
    {
        ...
        {
            oslot = (copen_slot **)hiarray_pop(node->importing);
            cluster_open_slot_destroy(*oslot);
        }
...
{
	cluster_slot *slot;
	slot = (cluster_slot *)hi_alloc(sizeof(*slot));
    if(slot == NULL){
        return NULL;
    }
    ...
    sds remote_name, cluster_node *node)
{
    ...
    oslot = (copen_slot *)hi_alloc(sizeof(*oslot));
    if(oslot == NULL){
        return NULL;
    }
    ...
    redisReply *port_elem, uint8_t role)
{
    ...
    node = (cluster_node *)hi_alloc(sizeof(cluster_node));
    if(node == NULL){
...
static void cluster_nodes_swap_ctx(dict *nodes_f, dict *nodes_t)
{
    ...
    while((de_t = dictNext(di)) != NULL){
        node_t = (cluster_node *)dictGetEntryVal(de_t);
        if(node_t == NULL){
            continue;
        }
        ...
        node_f = (cluster_node *)dictGetEntryVal(de_f);
        if(node_f->con != NULL){
            c = node_f->con;
    ...
    dict **nodes, cluster_node *node, sds master_name)
{
    ...
    {
        node_old = (cluster_node *)dictGetEntryVal(di);
        if(node_old == NULL)
        {
            __redisClusterSetError(cc,REDIS_ERR_OTHER,
                "dict get value null");
            ...
            if(idx == 0){
                ...
                if(idx == 2){
                    ...
                    if(den != NULL){
                        ...
                        master = (cluster_node *)dictGetEntryVal(den);
                    ...
                    if(ret != DICT_OK){
                        __redisClusterSetError(cc,REDIS_ERR_OTHER,
                            "The address already exists in the nodes");
                        cluster_node_deinit((cluster_node *)master);
                        hi_free(master);
                        goto error;
                    }
                    ...
                    if(master->slaves == NULL){
                        ...
                        if(master->slaves == NULL){
                            __redisClusterSetError(cc,REDIS_ERR_OOM,
                                "Out of memory");
                            cluster_node_deinit((cluster_node *)slave);
                            goto error;
                        }
                    ...
                    if(slot_start_end == NULL){
                        ...
                        if(flags & HIRCLUSTER_FLAG_ADD_OPENSLOT && 
                            ...
                            if(slot_start_end[1][0] == '>'){
                                ...
                                oslot_elem = (copen_slot**)hiarray_push(master->migrating);
                                if(oslot_elem == NULL){
                                    __redisClusterSetError(cc,REDIS_ERR_OTHER,
                                ...
                                *oslot_elem = oslot;
                                ...
                                oslot_elem = (copen_slot**)hiarray_push(master->importing);
                                if(oslot_elem == NULL){
                                    __redisClusterSetError(cc,REDIS_ERR_OTHER,
                                        "push migrating array error: out of memory");
                                    cluster_open_slot_destroy(oslot);
    ...
    const char *ip, int port)
{
    ...
    if(cc->flags & HIRCLUSTER_FLAG_ROUTE_USE_SLOTS){
        reply = (redisReply *)redisCommand(c, REDIS_COMMAND_CLUSTER_SLOTS);
        if(reply == NULL){
        ...
        reply = (redisReply *)redisCommand(c, REDIS_COMMAND_CLUSTER_NODES);
        if(reply == NULL){
            if (c->err == REDIS_ERR_TIMEOUT) {
                __redisClusterSetError(cc,c->err,
    ...
    while((den = dictNext(dit))){
        ...
        while((lnode = listNext(lit))){
            slot = (cluster_slot *)listNodeValue(lnode);
            if(slot->start > slot->end || 
            ...
            slot_elem = (cluster_slot**)hiarray_push(slots);
            *slot_elem = slot;
        }
    ...
    for(j = 0; j < hiarray_n(slots); j ++){
        slot_elem = (cluster_slot**)hiarray_get(slots, j);
        ...
        if(*pos == '\n')
        {
            ...
            if(role_len >= 6 && memcmp(role, "master", 6) == 0)
            {
                ...
                for(k = 8; k < count_part; k ++)
                {
                    ...
                    slot = (cluster_slot **)hiarray_push(slots);
                    if(slot == NULL)
                    {
                        __redisClusterSetError(cc,REDIS_ERR_OTHER,
...
static void print_cluster_node_list(redisClusterContext *cc)
{
    ...
    while((de = dictNext(di)) != NULL) {
        master = (cluster_node *)dictGetEntryVal(de);
        ...
        {
            slave = (cluster_node * )listNodeValue(ln);
...
redisClusterContext *redisClusterContextInit(void) {
    ...
    cc = (redisClusterContext *)calloc(1,sizeof(redisClusterContext));
    if (cc == NULL)
        return NULL;
...
static cluster_node *node_get_by_slot(redisClusterContext *cc, uint32_t slot_num)
{
    ...
    do{
        ...
        slot = (cluster_slot **)hiarray_get(slots, middle);
...
static cluster_node *node_get_witch_connected(redisClusterContext *cc)
{
    ...
    while((de = dictNext(di)) != NULL)
    {
        ...
        reply = (redisReply *)redisCommand(c, REDIS_COMMAND_PING);
    ...
    const char *config_name, int *config_value_len)
{
    ...
    reply = (redisReply *)redisCommand(c, "config get %s", config_name);
    if(reply == NULL)
    {
        __redisClusterSetError(cc, 
...
static int __redisClusterAppendCommand(redisClusterContext *cc, 
    ...
    if (__redisAppendCommand(c, command->command, command->clen) != REDIS_OK)
    {
        __redisClusterSetError(cc, c->err, c->errstr);
        return REDIS_ERR;
    }
...
static int __redisClusterGetReply(redisClusterContext *cc, int slot_num, void **reply)
{
    ...
    if(cluster_reply_error_type((redisReply *)(*reply)) == CLUSTER_ERR_MOVED)
    {
        cc->need_update_route = 1;
    }
...
ask_retry:
    ...
    error_type = cluster_reply_error_type((redisReply*)reply);
    if(error_type > CLUSTER_NOT_ERR && error_type < CLUSTER_ERR_SENTINEL)
    {
        ...
        case CLUSTER_ERR_ASK:
            node = node_get_by_ask_error_reply(cc, (redisReply *)reply);
            if(node == NULL)
            {
                freeReplyObject(reply);
                return NULL;
            ...
            reply = (redisReply *)redisCommand(c, REDIS_COMMAND_ASKING);
            if(reply == NULL)
            {
                __redisClusterSetError(cc, c->err, c->errstr);
                return NULL;
    ...
    struct cmd *command, hilist *commands)
{
    ...
    for (i = 0; i < REDIS_CLUSTER_SLOTS; i++) {     /* prepend command header */
        ...
        if (command->type == CMD_REQ_REDIS_MGET) {
            ...
            sub_command->command = (char*)hi_zalloc(sub_command->clen * sizeof(*sub_command->command));
            if(sub_command->command == NULL)
            {
                __redisClusterSetError(cc,REDIS_ERR_OOM,"Out of memory");
                slot_num = -1;
                goto done;
            ...
            sub_command->command[idx++] = '*';
            memcpy(sub_command->command + idx, num_str, num_str_len);
            idx += num_str_len;
            memcpy(sub_command->command + idx, "\r\n$4\r\nmget\r\n", 12);
            idx += 12;
            ...
            for(j = 0; j < hiarray_n(sub_command->keys); j ++)
            {
                ...
                memcpy(sub_command->command + idx, num_str, num_str_len);
                idx += num_str_len;
                memcpy(sub_command->command + idx, CRLF, CRLF_LEN);
                idx += CRLF_LEN;
                memcpy(sub_command->command + idx, kp->start, key_len);
                idx += key_len;
                memcpy(sub_command->command + idx, CRLF, CRLF_LEN);
                idx += CRLF_LEN;
            }
            ...
            for(j = 0; j < hiarray_n(sub_command->keys); j ++)
            {
                ...
                memcpy(sub_command->command + idx, CRLF, CRLF_LEN);
                idx += CRLF_LEN;
                memcpy(sub_command->command + idx, kp->start, key_len + kp->remain_len);
    ...
    struct cmd *command, hilist *commands)
{
    ...
    reply = (redisReply *)hi_calloc(1,sizeof(*reply));
    ...
    if (command->type == CMD_REQ_REDIS_MGET) {
        ...
        reply->element = (redisReply **)hi_calloc(key_count, sizeof(*reply));
        ...
        reply->str = (char *)hi_alloc((str_len + 1) * sizeof(char*));
        if(reply->str == NULL)
        {
            freeReplyObject(reply);
            __redisClusterSetError(cc,REDIS_ERR_OOM,"Out of memory");
    ...
    struct cmd *command, hilist *commands)
{
    ...
    if(cc == NULL || commands == NULL ||
        command == NULL || 
        command->command == NULL || command->clen <= 0)
    {
        goto done;
    }
...
void *redisClusterFormattedCommand(redisClusterContext *cc, char *cmd, int len) {
    ...
    {
        reply = (redisReply*)redis_cluster_command_execute(cc, command);
        goto done;
    }
    ...
    while((list_node = listNext(list_iter)) != NULL)
    {
        ...
        reply = (redisReply*)redis_cluster_command_execute(cc, sub_command);
        if(reply == NULL)
        {
            goto error;
        }
    ...
    reply = (redisReply*)command_post_fragment(cc, command, commands);
...
void *redisClustervCommand(redisClusterContext *cc, const char *format, va_list ap) {
    ...
    reply = (redisReply*)redisClusterFormattedCommand(cc, cmd, len);
...
int redisClusterGetReply(redisClusterContext *cc, void **reply) {
    ...
    *reply = NULL;
    ...
    while((list_sub_command = listNext(list_iter)) != NULL)
    {
        ...
        sub_command->reply = (redisReply*)sub_reply;
    }
...
static void __redisClusterAsyncSetError(redisClusterAsyncContext *acc, 
    ...
    if (str != NULL) {
        ...
        /* Only REDIS_ERR_IO may lack a description! */
        assert(type == REDIS_ERR_IO);
		__redis_strerror_r(error_number, acc->errstr, sizeof(acc->errstr));
    }
}
...
static redisClusterAsyncContext *redisClusterAsyncInitialize(redisClusterContext *cc) {
    ...
    acc = (redisClusterAsyncContext *)hi_alloc(sizeof(redisClusterAsyncContext));
    if (acc == NULL)
        return NULL;
...
static cluster_async_data *cluster_async_data_get(void)
{
    ...
    cad = (cluster_async_data *)hi_alloc(sizeof(cluster_async_data));
    if(cad == NULL)
    {
        return NULL;
    }
...
static void redisClusterAsyncCallback(redisAsyncContext *ac, void *r, void *privdata) {
    ...
    cluster_async_data *cad = (cluster_async_data *)privdata;
    redisClusterAsyncContext *acc;
    redisClusterContext *cc;
...
retry:
    ...
    ret = redisAsyncFormattedCommand(ac_retry,
        redisClusterAsyncCallback,cad,command->command,command->clen);
    if(ret != REDIS_OK)
    {
        goto error;
    }
...
int redisClusterAsyncFormattedCommand(redisClusterAsyncContext *acc, 
    ...
    command->command = (char*)malloc(len*sizeof(*command->command));
    if(command->command == NULL)
    {
        __redisClusterAsyncSetError(acc,REDIS_ERR_OOM,"Out of memory");
        goto error;
    }
    memcpy(command->command, cmd, len);
    command->clen = len;
