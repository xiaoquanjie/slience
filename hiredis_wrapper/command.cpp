...
redis_parse_cmd(struct cmd *r)
{
    ...
    state = SW_START;
    cmd_end = r->command + r->clen;

    ASSERT(state >= SW_START && state < SW_SENTINEL);
    ASSERT(r->command != NULL && r->clen > 0);

    for (p = r->command; p < cmd_end; p++) {
        ch = *p;
        ...
        case SW_KEY:
            ...
            if (*m != CR) {
                ...
                kpos = (keypos *)hiarray_push(r->keys);
                if (kpos == NULL) {
                    goto enomem;
                }
                kpos->start = m;
...
#else
	set_error_number(WSAEWOULDBLOCK);
#endif
    if(r->errstr == NULL){
...
{
    struct cmd *command;
    command = (cmd*)hi_alloc(sizeof(struct cmd));
    if(command == NULL)
    {
        return NULL;
    }
